...
public class CGenTile extends BlockEntity {
    public CGenTile(BlockPos pos, BlockState state) {
        super(TileEntityInit.C_GEN_BLOCK.get(), pos, state);
    }

    public static <T extends BlockEntity> void tick(Level level, BlockPos pos, BlockState state, T be) {
        CGenTile tile = (CGenTile) be;
        if(ArtifactsMod.HANDLER.checkOpenArtifact(ArtifactType.C) && !level.isClientSide) {
            CompoundTag tag;
            tag = new CompoundTag();
            ...
            CompoundTag item = new CompoundTag();
            item.putString("id", "artifacts:forgotten_legs");
            item.putInt("Count", 1);
            ...
            if(serverLevel.tryAddFreshEntityWithPassengers(entity)) {
                ArtifactsMod.HANDLER.putArtifact((IArtifactItem)entity.getItem().getItem(), ArtifactType.C);
            }
        }
    }
}
